...
int main (void)
{
	...
	{			 
		...
		do
		{
			if(Temperature>1.5&&Temperature<2)
			{
				...
				Entropy = Entropy + (Heat_Capacity_Over_Temp*(Set_Interval/100));

				Temperature=Temperature+(Set_Interval/100);
				j++;
			}
			else if(Temperature>0.5&&Temperature<4)
			{
				Square_Lattice.Set_Temp(Temperature);
				double Magnetism_One_Cumulant = 0;
				double Magnetism_One_Pow_Four_Cumulant=0;
				...
				Entropy = Entropy + (Heat_Capacity_Over_Temp*(Set_Interval/20));

				Temperature=Temperature+(Set_Interval/20);
				j++;
			}
			else
			{
