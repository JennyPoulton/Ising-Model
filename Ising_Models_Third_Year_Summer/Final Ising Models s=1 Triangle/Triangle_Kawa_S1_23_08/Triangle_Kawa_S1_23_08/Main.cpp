...
int main (void)
{
	...
	int No_Particles_Lattice_Three =Triangle_Lattice.Set_Up_Magnetism_Three();
		...
		for(double Temperature=Set_Final_Temp; Temperature<Set_Initial_Temp; Temperature=Temperature-Set_Interval)
		{
			...
			Output_Cool << Temperature << "\t" << Magnetism_One << "\t" << Magnetism_Two << "\t" << Magnetism_Three << "\t" << Susceptibility_One << "\t" << Susceptibility_Two << "\t" << Susceptibility_Three << "\t" << Energy << "\t"<< Heat_Capacity << "\t"<<  Heat_Capacity_Over_Temp << "\t" << Entropy*Diviser << "\t" << Prob1 << "\t" << Prob0 << "\t" << Prob_neg1 << endl;
