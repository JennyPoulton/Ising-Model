...
int main (void)
{
	...
	{			 
		...
		for(double Temperature=Set_Initial_Temp; Temperature<Set_Final_Temp; Temperature=Temperature+Set_Interval)
		{
			...
			Probs = Triangle_Lattice.Get_Probability();

			Output1 << Temperature << "\t" << Magnetism << "\t" << Binder_Cumulant << "\t" << Energy << "\t"<< Heat_Capacity << "\t"<< Susceptibility << "\t"<<  Heat_Capacity_Over_Temp << "\t" << Entropy << "\t" << Probs.Prob1 << "\t" << Probs.Prob0 << "\t" << Probs.Prob_neg1 << endl;
