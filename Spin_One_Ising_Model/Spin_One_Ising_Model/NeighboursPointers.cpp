...
System::System()
{
	...
	for(int x=0; x<(ISINGSIZE); x++)
	{
		...
		{
			Lattice[x][y].Set_Spin((int)(pow(-1,(double)x))*(int)(pow(-1,(double)y))); //set the main values
		}
	}
	...
	for(int y=0; y<ISINGSIZE; y++)
	{
		for(int x=0; x<ISINGSIZE; x++)
		{
			...
			Up[x][y]=&Lattice[x][(ISINGSIZE+x-1)%(ISINGSIZE)];		//set the surrounding lattice values 
